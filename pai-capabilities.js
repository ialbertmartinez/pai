...
function boot(){
  ...
  if(!mount){
    console.warn('[Perform Air] No #'+MOUNT_ID+' element found on this page.');
    return;
  }
...
const AIRCRAFT = ["ATR42", "ATR72", "Airbus A220", "Airbus A300", "Airbus A310", "Airbus A318", "Airbus A319", "Airbus A320", "Airbus A321", "Airbus A330", "Airbus A340", "Airbus A350", "Airbus A380", "Airbus A600", "Antonov AN-148", "BAC1-11", "Beechcraft 1900", "Bell 204", "Bell 206", "Boeing 707", "Boeing 717", "Boeing 720", "Boeing 727", "Boeing 737", "Boeing 737 Next Gen", "Boeing 747", "Boeing 757", "Boeing 767", "Boeing 777", "Boeing 787", "Bombardier BD-500", "Bombardier Global Express", "Bombardier Q400", "British Aero Especial 146", "British Aero Especial 31", "British Aero Especial 32", "CFM56-7B", "Canadair 215", "Canadair 415", "Canadian Regional Jet", "Canadian Regional Jet 100", "Canadian Regional Jet 170/190", "Canadian Regional Jet 200", "Canadian Regional Jet 400", "Canadian Regional Jet 440", "Canadian Regional Jet 600", "Canadian Regional Jet 700", "Canadian Regional Jet 900", "Casa", "Challenger 604", "Convair 440", "Dassault", "DeHavilland Dash 6", "DeHavilland Dash 7", "DeHavilland Dash 8", "Douglas Corporation 10", "Douglas Corporation 3", "Douglas Corporation 6", "Douglas Corporation 7", "Douglas Corporation 8", "Douglas Corporation 9", "Embraer 120", "Embraer 135", "Embraer 140", "Embraer 145", "Embraer 170", "Embraer 190", "Embraer Legacy 450", "Fairchild-Dornier", "Fokker 100", "Fokker 28", "Fokker 50", "Fokker 70", "Fokker F27", "GE-CF6", "Gulf Stream", "Hawker Beechcraft 800", "King Air 100 Series", "King Air 90 Series", "Lockheed 1011", "Lockheed Commercial C130", "Lockheed Electra", "Lockheed L382", "McDonald Douglas MD80", "McDonald Douglas MD83", "McDonald Douglas MD88", "McDonald Douglas MD90", "McDonnell Douglas MD11", "Metroliner", "Nihon", "SAAB", "Saab 200", "Saab 340", "Sikorsky Skycrane"];
const ATA_NAMES = {"03": "", "15": "", "16": "", "20": "Standard Practices", "21": "Air Conditioning", "22": "Auto Flight", "23": "Communications", "24": "Electrical Power", "25": "Equipment / Furnishings", "26": "Fire Protection", "27": "Flight Controls", "28": "Fuel", "29": "Hydraulic Power", "30": "Ice & Rain Protection", "31": "Indicating / Recording", "32": "Landing Gear", "33": "Lights", "34": "Navigation", "35": "Oxygen", "36": "Pneumatic", "38": "Water / Waste", "39": "Electrical Panels", "47": "Nitrogen Generation", "49": "Auxiliary Power", "50": "Cargo Compartments", "52": "Doors", "54": "Nacelles / Pylons", "55": "Stabilizers", "57": "Wings", "61": "Propellers", "71": "Power Plant", "72": "Engine", "73": "Engine Fuel & Control", "75": "Engine Air", "76": "Engine Controls", "77": "Engine Indicating", "78": "Exhaust", "79": "Oil", "80": "Starting", "82": "Water Injection", "84": "", "85": "Fuel Cell"};

/* Map UI sort keys -> database columns */
const SORT_COL = { pn:"part_number", nom:"nomenclature", ata:"ata_number", aircraft:"aircraft" };

async function fetchPage(){
  const p = new URLSearchParams();
  p.set("select","part_number,nomenclature,aircraft,ata_number,easa,caac,der_available");
  ...
  if(state.pn){
    if(PN_NORMALIZED){
      p.set("part_number_norm","ilike.*"+state.pn.replace(/[^A-Za-z0-9]/g,"")+"*");
    } else {
  ...
  const res = await fetch(SUPABASE_URL+"/rest/v1/"+TABLE+"?"+p.toString(), {
    headers:{
      "apikey": SUPABASE_KEY,
  ...
  return {
    rows: rows.map(r=>({
      pn:r.part_number, nom:r.nomenclature||"", aircraft:r.aircraft||"",
      ata:r.ata_number||"", faa:true, easa:!!r.easa, caac:!!r.caac, der:!!r.der_available
    })),
    total, ms
  };
}
...
/* ---------------------------------------------------------
   STATE
----------------------------------------------------------*/
const state={ keyword:"", aircraft:"", pn:"", ata:"", certs:new Set(), page:1, pageSize:20, sort:"pn", dir:1 };

/* populate dropdowns */
const acSel=document.getElementById('f-aircraft');
AIRCRAFT.forEach(a=>{const o=document.createElement('option');o.value=a;o.textContent=a;acSel.appendChild(o)});
const ataSel=document.getElementById('f-ata');
Object.keys(ATA_NAMES).sort().forEach(c=>{
  const o=document.createElement('option');o.value=c;
  o.textContent = ATA_NAMES[c] ? ("ATA "+c+" — "+ATA_NAMES[c]) : ("ATA "+c);
...
function hl(text,q){
  ...
  if(i<0) return text;
  return text.slice(0,i)+'<span class="pai-hl">'+text.slice(i,i+q.length)+'</span>'+text.slice(i+q.length);
}
function tickOrDash(v){return v?'<span class="pai-tick"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><path d="M20 6 9 17l-5-5"/></svg></span>':'<span class="pai-dash">—</span>'}
...
async function render(){
  ...
  try{
    ...
    document.getElementById('results-count').innerHTML='Could not load capabilities';
    document.getElementById('latency').innerHTML='';
    tbEl.innerHTML='<tr><td colspan="9"><div class="pai-empty">'+
      ...
      '<h3>Unable to reach the database</h3><p style="max-width:520px;margin:0 auto">'+String(err.message).replace(/</g,'&lt;')+'</p></div></td></tr>';
  ...
  const filtered = (state.keyword||state.aircraft||state.pn||state.ata||state.certs.size);
  const pages=Math.max(1,Math.ceil(total/state.pageSize));
  if(state.page>pages && pages>0){ state.page=pages; return render(); }
  ...
  tb.innerHTML=slice.map((r,idx)=>`
    <tr class="pai-match-pulse pai-datarow" data-row="${idx}">
      ...
      <td class="pai-nom">${hl(r.nom,state.keyword)}</td>
      <td class="pai-ata pai-hide-m">${r.ata}</td>
      <td class="pai-ac"><span class="pai-ac-name">${r.aircraft}</span></td>
      <td class="pai-cert pai-hide-m">${tickOrDash(r.faa)}</td>
      <td class="pai-cert pai-hide-m">${tickOrDash(r.easa)}</td>
      <td class="pai-cert pai-hide-m">${tickOrDash(r.caac)}</td>
      <td class="pai-cert pai-hide-m">${r.der?'<span class="pai-der-pill">DER</span>':'<span class="pai-dash">—</span>'}</td>
      <td><button class="pai-btn-quote" data-pn="${r.pn}" data-nom="${r.nom.replace(/"/g,'&quot;')}" data-ac="${r.aircraft}" data-ata="${r.ata}">Request Quote
  ...
  document.querySelectorAll('.pai-m-expand').forEach(btn=>btn.addEventListener('click',e=>{
    e.stopPropagation();
    const tr=btn.closest('tr'); const idx=tr.dataset.row;
    const detail=document.querySelector(`.pai-cert-detail[data-detail="${idx}"]`);
    const open=detail.classList.toggle('pai-open'); btn.classList.toggle('pai-open',open);
  }));
  ...
  if(lo>1){pageBtns+=`<button class="pai-pg" data-pg="1">1</button>`; if(lo>2)pageBtns+=`<span style="color:#9fb1c6;padding:0 2px">…</span>`}
  for(let p=lo;p<=hi;p++) pageBtns+=`<button class="pai-pg ${p===state.page?'on':''}" data-pg="${p}">${p}</button>`;
  if(hi<pages){ if(hi<pages-1)pageBtns+=`<span style="color:#9fb1c6;padding:0 2px">…</span>`; pageBtns+=`<button class="pai-pg" data-pg="${pages}">${pages}</button>`}
  ...
  document.querySelectorAll('.pai-pg[data-pg]').forEach(b=>b.addEventListener('click',()=>{
    const p=parseInt(b.dataset.pg);if(p>=1&&p<=pages){state.page=p;render();window.scrollTo({top:document.querySelector('.pai-results-wrap').offsetTop-20,behavior:'smooth'})}
  }));
}

/* ---- wire up controls ---- */
let kt;
document.getElementById('f-keyword').addEventListener('input',e=>{clearTimeout(kt);kt=setTimeout(()=>{state.keyword=e.target.value.trim();state.page=1;render()},120)});
document.getElementById('f-pn').addEventListener('input',e=>{clearTimeout(kt);kt=setTimeout(()=>{state.pn=e.target.value.trim();state.page=1;render()},120)});
document.getElementById('f-aircraft').addEventListener('change',e=>{state.aircraft=e.target.value;state.page=1;render()});
document.getElementById('f-ata').addEventListener('change',e=>{state.ata=e.target.value;state.page=1;render()});
document.getElementById('f-pagesize').addEventListener('change',e=>{state.pageSize=parseInt(e.target.value);state.page=1;render()});
document.querySelectorAll('.pai-chip[data-cert]').forEach(c=>c.addEventListener('click',()=>{
  const k=c.dataset.cert; c.classList.toggle('pai-on');
  if(state.certs.has(k))state.certs.delete(k);else state.certs.add(k);
  state.page=1;render();
}));
document.getElementById('btn-reset').addEventListener('click',()=>{
  state.keyword="";state.aircraft="";state.pn="";state.ata="";state.certs.clear();state.page=1;
  document.getElementById('f-keyword').value="";document.getElementById('f-pn').value="";
  document.getElementById('f-aircraft').value="";document.getElementById('f-ata').value="";
  document.querySelectorAll('.pai-chip.on').forEach(c=>c.classList.remove('pai-on'));
  render();
});
document.querySelectorAll('thead th[data-sort]').forEach(th=>th.addEventListener('click',()=>{
  ...
  document.querySelectorAll('thead th').forEach(x=>{x.classList.remove('pai-sorted');x.querySelector('.pai-arrow')&&(x.querySelector('.pai-arrow').textContent='▲')});
  th.classList.add('pai-sorted');th.querySelector('.pai-arrow').textContent=state.dir===1?'▲':'▼';
  render();
}));
document.getElementById('btn-export').addEventListener('click',()=>{
  const b=document.getElementById('btn-export');const o=b.innerHTML;
  b.innerHTML='✓ Export coming soon';setTimeout(()=>b.innerHTML=o,1800);
});
...
if(!qModal)return;
const qForm=document.getElementById('qmForm'), qSuccess=document.getElementById('qmSuccess');
let currentPart={};
function openQuote(data){
  ...
  qForm.style.display='';qSuccess.classList.remove('pai-show');
  ['qName','qCompany','qEmail','qPhone','qQty','qNotes'].forEach(id=>document.getElementById(id).value='');
  document.getElementById('qCond').value='';document.getElementById('qAog').checked=false;
  document.getElementById('qWebsite').value='';
  ['qName','qCompany','qEmail'].forEach(id=>document.getElementById(id).style.borderColor='');
  qModal.classList.add('pai-open');document.body.style.overflow='hidden';
  setTimeout(()=>document.getElementById('qName').focus(),100);
}
function closeQuote(){qModal.classList.remove('pai-open');document.body.style.overflow=''}

document.getElementById('tbody').addEventListener('click',e=>{
  const btn=e.target.closest('.pai-btn-quote');
  if(!btn)return;
  openQuote({pn:btn.dataset.pn,nom:btn.dataset.nom,ac:btn.dataset.ac,ata:btn.dataset.ata});
});
...
qModal.addEventListener('click',e=>{if(e.target===qModal)closeQuote()});
document.addEventListener('keydown',e=>{if(e.key==='Escape'&&qModal.classList.contains('pai-open'))closeQuote()});
...
document.getElementById('qmSubmit').addEventListener('click',async()=>{
  ...
  let ok=true;
  [['qName',name],['qCompany',company],['qEmail',email]].forEach(([id,v])=>{
    const el=document.getElementById(id);
    if(!v){el.style.borderColor='#cf5a5f';ok=false;}else{el.style.borderColor='';}
  });
  if(email && !/^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(email)){document.getElementById('qEmail').style.borderColor='#cf5a5f';ok=false;}
  if(!ok)return;
  ...
  btn.disabled=true; btn.textContent='Submitting…'; btn.style.opacity='.7';
  ...
  try{
    const res=await fetch(SUPABASE_URL+'/functions/v1/'+RFQ_FUNCTION,{
      method:'POST',
      headers:{
        'apikey':SUPABASE_KEY,
        'Authorization':'Bearer '+SUPABASE_KEY,
    ...
    document.getElementById('qmRef').textContent='Reference: '+(out.reference||'received');
    qForm.style.display='none'; qSuccess.classList.add('pai-show');
  }catch(err){
    console.error('RFQ submit failed:',err);
    if(errBox){
      errBox.textContent='Sorry — we couldn\u2019t submit that just now. Please try again, or email us directly. ('+err.message+')';
      errBox.style.display='block';
    }
  }finally{
    btn.disabled=false; btn.textContent=original; btn.style.opacity='';
  }
});
}
